class WeatherLivePro {
    ...
    return new Promise((resolve, reject) => {
      ...
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          console.log(
            `🎯 Position accuracy: ±${Math.round(position.coords.accuracy)}m`
          );
          resolve(position);
        },
        (error) => {
        ...
        },
        {
          ...
          timeout: 12000, // Longer timeout for better accuracy
  ...
  async loadWeatherByCoordinates(latitude, longitude) {
    try {
      ...
      // Track location by sending to server
      await this.trackUserLocation(latitude, longitude);
  ...
  // Enhanced method to track user location with maximum accuracy
  async trackUserLocation(latitude, longitude) {
    try {
      console.log("📍 Starting comprehensive location tracking...");

      // Get enhanced position data if available
      let enhancedLocationData = {
        latitude,
        longitude,
        timestamp: new Date().toISOString(),
        userAgent: navigator.userAgent,
        locationSource: "gps",
        // Additional client-side data collection
      ...
      // Try to get more accurate location data with all available GPS data
      try {
        const enhancedPosition = await this.getCurrentPositionEnhanced();
        if (enhancedPosition) {
          const coords = enhancedPosition.coords;
          enhancedLocationData = {
            ...enhancedLocationData,
            latitude: coords.latitude,
            longitude: coords.longitude,
            accuracy: coords.accuracy,
            altitude: coords.altitude, // May be null if not available
            altitudeAccuracy: coords.altitudeAccuracy, // May be null
            heading: coords.heading, // May be null if device is stationary
            speed: coords.speed, // May be null if device is stationary
            timestamp: new Date(enhancedPosition.timestamp).toISOString(),
            locationSource: "enhanced_gps",
          };

          console.log(`🎯 Enhanced GPS data collected:`);
          console.log(`   - Accuracy: ±${Math.round(coords.accuracy)}m`);
          console.log(
            `   - Altitude: ${
              coords.altitude !== null
                ? Math.round(coords.altitude) + "m"
                : "Not available (no barometric sensor)"
            }`
          );
          console.log(
            `   - Heading: ${
              coords.heading !== null
                ? Math.round(coords.heading) + "° (compass direction)"
                : "Not available (device stationary/no compass)"
            }`
          );
          console.log(
            `   - Speed: ${
              coords.speed !== null
                ...
                : "Not available (device stationary)"
            }`
          );
        }
      } catch (enhancedError) {
        console.warn("⚠️ Enhanced GPS failed, using basic coordinates");
      }

      // Get timezone information
      try {
        enhancedLocationData.timezone =
          Intl.DateTimeFormat().resolvedOptions().timeZone;
        console.log(`🕐 Timezone detected: ${enhancedLocationData.timezone}`);
      } catch (tzError) {
        console.warn("⚠️ Could not get timezone");
      }

      // Get detailed location information from Google Geocoding
      try {
        ...
        if (locationInfo) {
          ...
          console.log(
            `🏙️ Location details: ${locationInfo.city}, ${locationInfo.country}`
          );
          console.log(`📍 Address: ${locationInfo.formatted_address}`);
        }
      } catch (locationError) {
        console.warn("⚠️ Could not get detailed location info");
        enhancedLocationData.cityName = "Unknown";
        enhancedLocationData.country = "Unknown";
      }

      // Log comprehensive data being sent
      console.log("📊 Comprehensive tracking data being sent:");
      const dataCategories = {
        "GPS Core": ["latitude", "longitude", "accuracy"],
        "GPS Enhanced": ["altitude", "altitudeAccuracy", "heading", "speed"],
        "Location Info": [
          "timezone",
          "cityName",
          "country",
        ...
        Technical: ["userAgent", "locationSource", "timestamp"],
      };

      Object.entries(dataCategories).forEach(([category, fields]) => {
        console.log(`   ${category}:`);
        fields.forEach((field) => {
          const value = enhancedLocationData[field];
          if (value !== null && value !== undefined) {
            console.log(`      ✅ ${field}: ${value}`);
          } else {
            console.log(`      ⚠️ ${field}: NULL (not available)`);
          }
        });
      });

      // Send comprehensive data to server
      ...
      if (response.ok) {
        ...
        console.log("✅ Comprehensive location tracking successful");

        // Log data completeness results
        if (result.data.dataCompleteness) {
          const completeness = result.data.dataCompleteness;
          console.log("📈 Data completeness summary:");
          console.log(`   GPS fields: ${completeness.gpsFields}/6 populated`);
          console.log(
            `   Location fields: ${completeness.locationFields}/4 populated`
          );
          console.log(
            `   Device fields: ${completeness.deviceFields}/5 populated`
          );
          console.log(`   IP fields: ${completeness.ipFields}/9 populated`);
          console.log(
            `   Total: ${completeness.totalFieldsPopulated}/${completeness.totalPossibleFields} fields`
          );
        }
        ...
        if (result.data.stored) {
          console.log(
            "💾 Successfully saved to database with comprehensive data"
          );
        } else if (result.data.dbError) {
          console.error("❌ Database save failed:", result.data.dbError);
          console.error("💡 Check server logs and run database schema update");
        }
      } else {
        const errorData = await response.json().catch(() => ({}));
        console.warn("⚠️ Location tracking failed:", errorData);
      }
    } catch (error) {
      console.warn(
        "⚠️ Comprehensive location tracking error (non-critical):",
        error
      );
    }
  }

  // Enhanced GPS positioning with better error handling
  getCurrentPositionEnhanced() {
    return new Promise((resolve, reject) => {
      ...
      console.log(
        "🎯 Requesting enhanced geolocation with all available data..."
      );

      navigator.geolocation.getCurrentPosition(
        (position) => {
          const coords = position.coords;
          console.log("✅ Enhanced geolocation success - Available data:");
          console.log(`   - Latitude: ${coords.latitude}`);
          console.log(`   - Longitude: ${coords.longitude}`);
          console.log(`   - Accuracy: ±${Math.round(coords.accuracy)}m`);

          // Log availability of optional GPS data
          if (coords.altitude !== null) {
            console.log(
              `   - Altitude: ${Math.round(coords.altitude)}m (±${
            ...
            console.log("   - Altitude: Not available (device limitation)");
          }

          if (coords.heading !== null) {
            console.log(
              `   - Heading: ${Math.round(coords.heading)}° (compass direction)`
            );
          } else {
            console.log(
              "   - Heading: Not available (device stationary/no compass)"
            );
          }

          if (coords.speed !== null) {
            console.log(`   - Speed: ${Math.round(coords.speed * 3.6)} km/h`);
          } else {
            console.log("   - Speed: Not available (device stationary)");
          }

          resolve(position);
        },
        (error) => {
          console.warn("⚠️ Enhanced geolocation failed:", error.message);
          reject(error);
        },
        {
          enableHighAccuracy: true, // Use GPS for maximum accuracy
          timeout: 15000, // Wait longer for accurate fix
  ...
  // New method to get detailed location info
  async getDetailedLocationInfo(lat, lng) {
    try {
      // Use the server's geocoding to avoid exposing API key
